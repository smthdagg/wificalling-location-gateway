...
var ZH = {
	...
	'Fair': '一般',
	'Follow gateway': '跟随网关',
	'WLOC target device': 'WLOC 目标设备',
	'WLOC target device IPv4': 'WLOC 目标设备 IPv4',
	'WLOC location interception: rewrites the Apple WLOC response for the selected device. GPS values stay on this router.': 'WLOC 定位拦截：仅改写选定设备的 Apple WLOC 响应，GPS 数值只保存在本路由器。',
	'WLOC owns this device scope. Auto mode only reads the WFC node exit IP; manual mode does not use WFC. Leave empty to keep WLOC disabled/pass-through.': 'WLOC 自己管理设备范围。自动模式只读取 WFC 节点出口 IP，手动模式不使用 WFC。留空则保持禁用/直通。',
	'Only independent tunnel is supported. Legacy follow-gateway entries are rejected with a startup diagnostic.': '仅支持独立通道；旧的跟随网关配置会在启动时明确报错。',
