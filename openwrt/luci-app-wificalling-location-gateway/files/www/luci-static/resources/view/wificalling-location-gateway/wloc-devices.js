...
function profileFromFields(section, fields) {
	return { '.name': section['.name'], label: fields.label.value.trim(), assigned_device: fields.address.value.trim(), node_ref: fields.node.value.trim(), node_mode: 'fixed', geo_source: fields.geoMode.value, manual_lat: fields.latitude.value.trim(), manual_lon: fields.longitude.value.trim(), enabled: fields.enabled.checked ? '1' : '0' };
}
...
return view.extend({
	...
	render: function(data) {
		...
		function refreshHealth(current) { current = current || {}; var services = (current || {}).services || {}, wloc = services.wloc || {}, provider = services.provider || {}; healthSummary.textContent = 'WLOC: ' + (wloc.running ? 'running' : 'stopped') + ' | Provider: ' + (provider.valid ? 'ready' : 'unavailable') + ' | ' + (wloc.phase || 'unknown'); (current.profiles || []).forEach(function(profile) { if (stateCells[profile.id]) stateCells[profile.id].textContent = profileStatus(profile); }); }
		...
		function applyAll() { if (!stageBasic()) return Promise.resolve(false); var errors = validateProfiles(uci.sections('wloc-service', 'device')); if (errors.length) { notify(wlocI18n.t('Apply failed'), errors.join('; '), 'error'); return Promise.resolve(false); } return uci.save('wloc-service').then(function() { return ui.changes.apply(true); }).then(function() { return restartUnified(); }).then(function(result) { if (result && result.error) throw new Error(result.error); notify(wlocI18n.t('Applied'), wlocI18n.t('Standalone WLOC supervisor restarted.')); return L.resolveDefault(getHealth(), {}).then(refreshHealth); }).catch(function(error) { notify(wlocI18n.t('Apply failed'), String(error), 'error'); return false; }); }
		function renderRows() { body.innerHTML = ''; stateCells = {}; var profiles = uci.sections('wloc-service', 'device'); if (!profiles.length) { body.appendChild(E('tr', {}, E('td', { colspan: 8 }, wlocI18n.t('No device profiles yet.')))); return; } profiles.forEach(function(section) { var fields = { label: textInput(section.label, wlocI18n.t('Label')), address: textInput(section.assigned_device, '192.168.1.100 or MAC'), node: textInput(section.node_ref || 'default', 'WLOC node reference'), latitude: textInput(section.manual_lat, 'lat'), longitude: textInput(section.manual_lon, 'lon'), enabled: E('input', { type: 'checkbox', checked: section.enabled === '1' }), geoMode: E('select', {}) }; setOption(fields.geoMode, [['auto', 'Auto follow selected node'], ['manual', 'Manual location']], section.geo_source || 'auto'); var state = E('span', {}); stateCells[section['.name']] = state; state.textContent = profileStatus((health.profiles || []).filter(function(p) { return p.id === section['.name']; })[0]); body.appendChild(E('tr', { class: 'tr' }, [E('td', { class: 'td' }, section['.name']), E('td', { class: 'td' }, fields.label), E('td', { class: 'td' }, fields.address), E('td', { class: 'td' }, fields.node), E('td', { class: 'td' }, fields.geoMode), E('td', { class: 'td' }, [fields.latitude, ' ', fields.longitude]), E('td', { class: 'td' }, [fields.enabled, ' ', state]), E('td', { class: 'td' }, [E('button', { class: 'cbi-button cbi-button-apply', click: function() { stageProfile(section, fields); } }, wlocI18n.t('Stage')), ' ', E('button', { class: 'cbi-button cbi-button-remove', click: function() { removeProfile(section); } }, wlocI18n.t('Delete'))])])); }); }
		...
		return E([], [E('h2', {}, wlocI18n.t('WLOC Devices')), E('p', {}, wlocI18n.t('Each device owns one explicit WLOC node, one auto/manual location policy, service enablement, and bounded status.')), E('div', { class: 'cbi-section' }, [E('p', {}, healthSummary), E('p', {}, [E('button', { class: 'cbi-button cbi-button-add', click: addProfile }, wlocI18n.t('Add profile')), ' ', applyButton]), E('table', { class: 'table' }, [E('tr', { class: 'tr table-titles' }, ['ID', wlocI18n.t('Label'), wlocI18n.t('Device'), wlocI18n.t('WLOC node'), wlocI18n.t('Location mode'), wlocI18n.t('Manual location'), wlocI18n.t('Enabled / state'), wlocI18n.t('Action')].map(function(title) { return E('th', { class: 'th' }, title); })), body])])]);
	}
});
