...
return view.extend({
	...
	render: function(data) {
		...
		function applyAll() { if (!stageBasic()) return Promise.resolve(false); var errors = validateProfiles(uci.sections('wloc-service', 'device')); if (errors.length) { notify(wlocI18n.t('Apply failed'), errors.join('; '), 'error'); return Promise.resolve(false); } return uci.save('wloc-service').then(function() { return ui.changes.apply(true); }).then(function() { return restartUnified(); }).then(function(result) { if (result && result.error) throw new Error(result.error); notify(wlocI18n.t('Applied'), wlocI18n.t('Unified Gateway/WLOC supervisor restarted.')); return getHealth().then(refreshHealth); }).catch(function(error) { notify(wlocI18n.t('Apply failed'), String(error), 'error'); return false; }); }
