...
edits = [
    ...
    (
        ...
        "\t} > \"$cfg\"\n"
        "\tsingbox_helper=\"${WIFICALLING_SINGBOX_RUNTIME:-/usr/libexec/wificalling-location-gateway/singbox-runtime.sh}\"\n"
        "\tsingbox_bin=$(\"$singbox_helper\" path 2>/dev/null || true)\n"
        "\tif [ -z \"$singbox_bin\" ]; then\n"
        "\t\tprintf '%s\\nfailed\\nprovider_missing\\n' \"$(date +%s)\" > \"$cache\"\n"
        "\t\trm -f \"$cfg\"; rm -f \"$lock/pid\"; rmdir \"$lock\" 2>/dev/null || true\n"
        "\t\treturn 1\n"
        "\tfi\n"
