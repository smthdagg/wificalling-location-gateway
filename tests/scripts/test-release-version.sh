...
grep -Fx 'PKG_VERSION:=1.0.10' "$repo_root/openwrt/luci-app-wificalling-location-gateway/Makefile" >/dev/null ||
	fail 'LuCI package version must be 1.0.10'
...
for expected in \
	'wificalling-location-gateway_1.0.10-r1_x86_64.ipk' \
	'wificalling-location-gateway-1.0.10-r1.apk'; do
	printf '%s\n' "$plan" | grep -F "$expected" >/dev/null ||
		fail "release plan is missing $expected"
done

grep -F 'version=${1:-1.0.10-1}' "$repo_root/scripts/build-luci-ipk.sh" >/dev/null ||
	fail 'AX6S standalone builder default must be 1.0.10 release 1'
