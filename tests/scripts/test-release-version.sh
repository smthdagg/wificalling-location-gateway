...
grep -Fx 'PKG_RELEASE:=14' "$repo_root/openwrt/Makefile" >/dev/null ||
...
grep -Fx 'PKG_RELEASE:=14' "$repo_root/openwrt/luci-app-wificalling-location-gateway/Makefile" >/dev/null ||
...
for expected in \
	'wificalling-location-gateway_1.3.0-r14_x86_64.ipk' \
	'wificalling-location-gateway-1.3.0-r14.apk'; do
	printf '%s\n' "$plan" | grep -F "$expected" >/dev/null ||
		fail "release plan is missing $expected"
done

grep -F 'version=${1:-1.3.0-r14}' "$repo_root/scripts/build-luci-ipk.sh" >/dev/null ||
