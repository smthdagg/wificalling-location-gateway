...
[ "$(cat "$repo_root/VERSION")" = 2.0.0 ] || fail 'canonical version must be 2.0.0'
grep -Eq '^version = "2\.0\.0"$' "$repo_root/Cargo.toml" ||
	fail 'Cargo package version must be 2.0.0'
grep -F 'webpki-roots = "=1.0.9"' "$repo_root/Cargo.toml" >/dev/null ||
	fail 'version bumps must not rewrite pinned dependency versions'
grep -Fx 'PKG_VERSION:=2.0.0' "$repo_root/openwrt/Makefile" >/dev/null ||
	fail 'OpenWrt runtime version must be 2.0.0'
grep -Fx 'PKG_RELEASE:=1' "$repo_root/openwrt/Makefile" >/dev/null ||
	fail 'OpenWrt runtime release must reset to 1'
grep -Fx 'PKG_VERSION:=2.0.0' "$repo_root/openwrt/luci-app-wificalling-location-gateway/Makefile" >/dev/null ||
	fail 'LuCI package version must be 2.0.0'
...
for expected in \
	'wificalling-location-gateway_2.0.0-r1_x86_64.ipk' \
	'wificalling-location-gateway-2.0.0-r1.apk'; do
