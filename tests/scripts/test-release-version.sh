...
[ "$(cat "$repo_root/VERSION")" = 1.0.11 ] || fail 'canonical version must be 1.0.11'
grep -Eq '^version = "1\.0\.11"$' "$repo_root/Cargo.toml" ||
	fail 'Cargo package version must be 1.0.11'
grep -F 'webpki-roots = "=1.0.9"' "$repo_root/Cargo.toml" >/dev/null ||
	fail 'version bumps must not rewrite pinned dependency versions'
grep -Fx 'PKG_VERSION:=1.0.11' "$repo_root/openwrt/Makefile" >/dev/null ||
	fail 'OpenWrt runtime version must be 1.0.11'
grep -Fx 'PKG_RELEASE:=1' "$repo_root/openwrt/Makefile" >/dev/null ||
	fail 'OpenWrt runtime release must reset to 1'
grep -Fx 'PKG_VERSION:=1.0.11' "$repo_root/openwrt/luci-app-wificalling-location-gateway/Makefile" >/dev/null ||
	fail 'LuCI package version must be 1.0.11'
...
for expected in \
	'wificalling-location-gateway_1.0.11-r1_x86_64.ipk' \
	'wificalling-location-gateway-1.0.11-r1.apk'; do
	printf '%s\n' "$plan" | grep -F "$expected" >/dev/null ||
		fail "release plan is missing $expected"
done

grep -F 'version=${1:-1.0.11-1}' "$repo_root/scripts/build-luci-ipk.sh" >/dev/null ||
	fail 'AX6S standalone builder default must be 1.0.11 release 1'
