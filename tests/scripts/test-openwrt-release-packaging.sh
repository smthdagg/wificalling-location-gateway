...
printf '%s\n' "$plan" | grep -F 'wificalling-location-gateway_1.0.4-r1_x86_64.ipk' >/dev/null ||
	fail '24.10 must produce one architecture-specific integrated IPK'
printf '%s\n' "$plan" | grep -F 'wificalling-location-gateway-1.0.4-r1.apk (arch: x86_64)' >/dev/null ||
...
if printf '%s\n' "$plan" | grep -E 'wloc-service[_-]|luci-app-wificalling-location-gateway[_-]' >/dev/null; then
	fail 'formal 1.0.4 plan must not expose split component packages'
fi
