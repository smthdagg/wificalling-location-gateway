...
> 当前强制基线：本仓库以 `1.3.0-r1` 发布线（在 1.2.x 稳定整合包基础上演进）为唯一基线，后续所有修订、升级与代码改动均基于此继续延伸。早期 1.7 开发阶段的记录到此做结尾，仅作为已完成设计的历史来源追溯，不再是可用的构建输入、依赖或回退目标，后续工作不再引用 1.7 作为基线或外部依赖。多设备/2.0 Beta 已迁至独立仓库，不属于本项目的代码、文档、测试或发布范围；任何实现不得从该项目回迁。
