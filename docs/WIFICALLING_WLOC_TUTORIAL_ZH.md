...
自动模式使用节点出口 IP 的国家、城市、时区和坐标。启用自动模式时获取一次定位；健康轮询只在缓存出口证据过期时核对 IP，只有 IP 变化才重新获取 Geo 并更新目标。更换设备绑定节点后，在监控页点击“刷新 IP”可立即重探测。
...
确认第一部分的设备策略绑定了预期节点，Passwall 等全局代理已绕过测试设备，手机没有运行其他 VPN。切换节点后点击“刷新 IP”重新探测；`Geo state` 更新为 `fresh` 后即可。
