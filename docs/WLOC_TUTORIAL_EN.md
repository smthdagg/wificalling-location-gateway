...
1. Install the matching OpenWrt package and retain the sing-box tiny/lite or
   PassWall provider.
2. Open Services → WiFi Calling + WLOC Gateway → WiFi Calling Gateway. Add
   nodes and Gateway device routes as required by the local network.
3. Open WLOC Basic Settings. Enable WLOC, choose the Geo provider, and
   confirm the provider configuration path. When Gateway is enabled, WLOC
   reuses the Gateway-generated sing-box configuration.
4. Open Devices and create one WLOC profile per authorized LAN device. Enter its
...
6. Apply the change and confirm Gateway Status, WLOC Overview, and Service
   Status show the child daemons, provider, redirect, and profile as healthy.
