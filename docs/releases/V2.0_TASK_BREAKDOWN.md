...
Status: active redesign and implementation under GitHub Issue #41; the project
lead owns integration and acceptance. This document is the implementation
checklist, not a release approval. The product boundary is standalone WLOC;
ADR 0003 supersedes the earlier Gateway/WLOC integration wording.

## Current implementation checkpoint

Completed on the V2-09 branch but subject to boundary rework and re-verification:
...
- the current branch still has Gateway UCI/package/menu/build coupling; remove it
  before any final package is called standalone;
- merge global WLOC settings into device profiles and define explicit WLOC node
  targets; remove `gateway_default` / “Follow gateway” semantics;
- move Component Update to its own page and add device/firmware/architecture/
  package-format preflight;
- replace hard-coded/ad-hoc UI language handling with English source strings and
  the project LuCI language package;

- AX6S staging measurements now cover RSS, persistent storage, migration,
  restart, signed update, interrupted-update recovery, and reboot persistence;
  the redacted evidence is recorded in
  `docs/testing/AX6S_REAL_DEVICE_2026-08-22.md`. A repeatable CPU sample,
  real Wi-Fi Calling traffic, and client-isolation packet evidence remain
  pending;
- the architecture-correct OpenWrt cross-build is passing in PR CI; final
  standalone release package artifacts and their signed checksums still need to
  be generated;
- migration rehearsal from the installed 1.2.x package and a real-device
  rollback observation remain release gates.
