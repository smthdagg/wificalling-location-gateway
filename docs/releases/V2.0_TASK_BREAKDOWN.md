...
itself a release approval. The product boundary is standalone WLOC; ADR 0003
supersedes the earlier Gateway/WLOC integration wording. The signed V2 feed is
prepared locally; gh-pages/tag/release publication remains after PR merge and
...
- shared sing-box provider resolution that prefers AX6S-tested tiny/lite or
  PassWall binaries without adding a duplicate package payload. This provider
  is optional and must not be confused with a Wi-Fi Calling dependency.

Release acceptance status:

- package, UCI, init, RPC, LuCI, and build paths are standalone WLOC paths;
  no Gateway package, UCI, menu, binary, or runtime dependency is an input;
...
- AX6S evidence covers migration, provider reuse, RSS/storage, reboot,
  fail-open, signed update, successful update, and health-failure rollback;
- the architecture-correct three-package release build, SHA-256 verification,
  and four-environment Docker install matrix pass;
- real iPhone WLOC traffic, hard power-cut recovery during opkg, and flash-full
...
| V2-05 | Standalone LuCI settings, device pages, status, and diagnostics | integration | engine + test | V2-01, V2-02 |
