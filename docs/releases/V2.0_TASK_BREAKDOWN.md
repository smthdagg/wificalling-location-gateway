...
Status: implementation complete pending external publication approval under
GitHub Issue #41; the project lead owns integration and acceptance. This
document is the implementation checklist, not a release approval. The product
boundary is standalone WLOC; ADR 0003 supersedes the earlier Gateway/WLOC
integration wording.
...
- English source strings are used by the LuCI pages; Chinese translations are
  a separate follow-up language-package task;
...
- real iPhone WLOC traffic, hard power-cut recovery during opkg, and flash-full
  fault injection remain explicitly untested. Signed feed publication and
  external release approval are outside this local worktree.
