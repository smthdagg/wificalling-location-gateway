...
Status: active implementation under GitHub Issue #41; the project lead owns
integration and acceptance. This document is the implementation checklist,
not a release approval.

## Current implementation checkpoint

Completed on the V2-09 integration branch and covered by focused tests:
...
- unified supervisor health checks bound to the real WLOC socket, proxy listener,
  exact service PID, Gateway config and Gateway-owned nft table;
...
- shared sing-box provider resolution that prefers AX6S-tested tiny/lite or
  PassWall binaries without adding a duplicate package payload.
...
- AX6S staging measurements now cover RSS, persistent storage, migration,
  ...
  pending;
- the architecture-correct OpenWrt cross-build is passing in PR CI, but final
  release package artifacts and their signed checksums still need to be
  generated and recorded; the final accepted package version must be bumped
  from the `1.2.0` staging baseline to `2.0.0`;
